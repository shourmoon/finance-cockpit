...
//
// CORS:
//   - Allows browser calls from localhost / any origin (SPA usage)
//   - Handles OPTIONS preflight
//
// Security (PIN-gated):
//   - Client MUST send header: X-Sync-Pin = sha256(pin) (hex string)
//   - Worker stores first-seen pin hash per sharedKey and enforces it thereafter
//   - PIN hash is stored at KV key "${sharedKey}:pin"
//
// Concurrency (optimistic):
//   - If a snapshot already exists, PUT must include prev_updated_at
//   - If prev_updated_at does not match stored updated_at, worker returns 409
...
async function requirePin(env: any, sharedKey: string, request: Request): Promise<Response | null> {
  ...
  if (!stored) {
    // First use: bind this key to the provided pin hash.
...
export default {
  async fetch(request: Request, env: any): Promise<Response> {
    ...
    if (request.method === "PUT") {
      ...
      // Concurrency rules:
      // - If current exists, prev_updated_at is required and must match
      if (current) {
        if (!payload.prev_updated_at) {
          return withCors(new Response("Conflict", { status: 409 }));
        }
        if (current.updated_at !== payload.prev_updated_at) {
          return withCors(new Response("Conflict", { status: 409 }));
        }
      }
