...
- **Cashflow projection** — recurring income/expense rules (monthly, twice-a-month with US-Fed business-day adjustment, or biweekly) plus ad-hoc one-time transactions, expanded over a configurable horizon into a daily balance timeline, with per-occurrence amount overrides. The dashboard reads that timeline as figures rather than a curve: the minimum balance and when it lands, a month-grouped ledger of upcoming events with a running balance on every row, and a one-tap quick-add for one-off transactions.
...
- **One-salary coverage** — every top-up is recorded, and the dashboard keeps the running score: how many of the last twelve months the primary salary covered unaided, the current and best clean-month streak, the average monthly gap, and optionally how much of a second salary stayed in savings. Every draw counts the same, whatever prompted it — a month that needed money moved in did not run on one salary either way.
