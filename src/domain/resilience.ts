...
//
// Every applied top-up is a recorded event ({ kind: "topUp" }), and these
// functions roll them into monthly buckets and the six metrics the dashboard
// card shows. Every top-up counts, with no lens to view some of them alone:
// the question is whether one salary covered the month unaided, and a month
// that needed money moved in did not, whatever prompted the transfer.
//
// Months are calendar months, including the current, in-progress one: this
// app forecasts ahead, so a top-up is one the projection usually flagged
// before it was recorded rather than a month-end surprise — not a reason to
// hold back real, already-recorded data. The current month is flagged
// `complete: false` so the UI can mark it as still open without excluding it
// from any total or streak. Months before `trackingSince` are *unknown*
// rather than clean: the app wasn't recording then, and absence of data is
// not evidence of coverage.

import type { AdhocTransaction, ISODate, Money } from "./types";
...
export interface MonthBucket {
  /** "YYYY-MM". */
  monthKey: string;
  /** Every top-up recorded in this month. */
  total: Money;
...
export interface CoverageMetrics {
  ...
  cleanMonths: number;
  /** Sum of top-ups across the window. */
  totalToppedUp: Money;
...
export function computeCoverageMetrics(
  ...
  const { asOf, trackingSince, secondSalaryMonthly } = options;
